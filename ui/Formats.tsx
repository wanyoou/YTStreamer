...
export async function getValueByFormat(category: string, format: string): Promise<string | undefined> {
          ...
          {formats.map((cate) => {
            return (
              <div key={cate.category} className='space-y-2'>
                <label className='label text-sm font-semibold p-0'>{cate.category}</label>
                  ...
                  {cate.items.map((item) => {
                    return (
                      <button
                        ...
                        className={clsx('btn rounded-md w-14 h-14 flex-col place-content-evenly', {
                          'btn-active': group === cate.category && format === item.format,
                        })}
                      >
