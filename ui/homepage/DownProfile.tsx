    ...
    <div className='grid grid-cols-6 gap-x-2 gap-y-4 bg-base-200 rounded-md p-2'>
      ...
      <div className='form-control relative col-span-3'>
        ...
        >
          ...
          <ul tabIndex={0} className='dropdown-content menu menu-compact bg-base-200 w-fit rounded-box shadow'>
            {outputTemplate.map((output) => {
              return (
                <li key={output.name} onClick={() => setFilename(filename + output.value)} className='hover-bordered'>
                  <a className='px-2 py-1'>{output.name}</a>
                </li>
              );
            })}
            <li key='more' className='hover-bordered'>
              <a href='https://github.com/yt-dlp/yt-dlp#output-template' className='link link-info ps-2 py-1 gap-x-1.5'>
                MORE
                <svg
